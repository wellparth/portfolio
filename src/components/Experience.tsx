...
export default function Experience() {
  return (
    <section id="experience" className="max-w-5xl mx-auto px-4 sm:px-6 py-6 space-y-8">
      {/* Experience Header */}
      <h2 className="text-2xl font-serif font-bold text-ink-900 border-b border-beige-300 pb-3">
        ...
        {resumeData.experience.map((exp, idx) => (
          <div key={idx} className="beige-card space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 border-b border-beige-200 pb-3">
              <div>
                <h3 className="text-lg font-serif font-bold text-ink-900">{exp.role}</h3>
                <span className="text-xs font-semibold text-olive-600 uppercase tracking-wider font-sans">{exp.company}</span>
              </div>
            ...
            <ul className="space-y-2 text-xs font-sans text-ink-700 leading-relaxed">
      ...
      <div id="education" className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4">
        ...
        <div className="beige-card space-y-3">
          <h3 className="text-lg font-serif font-bold text-ink-900 border-b border-beige-200 pb-2">Education</h3>
          <div>
            <h4 className="text-sm font-semibold text-ink-900">{resumeData.education.institution}</h4>
            <p className="text-xs text-olive-600 font-semibold mt-0.5">{resumeData.education.degree}</p>
        ...
        {/* Languages */}
        <div className="beige-card space-y-3">
          <h3 className="text-lg font-serif font-bold text-ink-900 border-b border-beige-200 pb-2">Languages</h3>
          <div className="grid grid-cols-3 gap-2">
            {resumeData.languages.map((lang, lIdx) => (
              <div key={lIdx} className="bg-beige-200/50 p-2.5 rounded-lg border border-beige-300 text-center">
                ...
                <span className="text-[11px] text-olive-600 font-semibold">{lang.proficiency}</span>
              </div>
            ))}
          </div>
        </div>
