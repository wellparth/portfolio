export const resumeData = {
  personal: {
    name: "Parth Gorde",
    title: "Digital Project Coordinator & Operations Specialist",
    email: "wellparth@outlook.com",
    phone: "+91 90675 51734",
    linkedin: "https://www.linkedin.com/in/parthgorde",
    location: "Nashik / Pune, MH, India",
    status: "Available for Operations & Project Coordinator Roles",
    summary: "Operations and Project Coordination professional with a degree in Information Technology. Proven track record coordinating clients, engineering teams, and freelance talent through AI-assisted workflows to deliver web projects on time and within scope.",
  },
  metrics: [
    { value: "17+", label: "Web Projects Coordinated", detail: "End-to-end management from intake to deployment", color: "from-indigo-500 to-indigo-600" },
    { value: "15+", label: "Freelancers Managed", detail: "Built and coordinated cross-functional talent network", color: "from-cyan-500 to-cyan-600" },
    { value: "12+", label: "Business Clients", detail: "Scoping, quotations & client relationship management", color: "from-emerald-500 to-emerald-600" },
    { value: "B.E. IT", label: "Tech Engineering Degree", detail: "MET's Institute of Engineering (SPPU, 2020-2024)", color: "from-amber-500 to-amber-600" },
  ],
  workflowSteps: [
    {
      step: "01",
      title: "Discovery & Scoping",
      desc: "Gathering business requirements from prospective clients, clarifying deliverables, preparing quotations, and setting expectations.",
      tags: ["Client Intake", "Quotation Prep", "Scope Specification"]
    },
    {
      step: "02",
      title: "Talent & Sprint Setup",
      desc: "Matching project needs with developers, designers, or internal teams. Establishing timeline benchmarks and task priorities.",
      tags: ["Talent Matchmaking", "Agile Sprints", "Resource Allocation"]
    },
    {
      step: "03",
      title: "Execution & AI Automation",
      desc: "Tracking daily progress in Jira/Notion, identifying blockers early, and leveraging Claude/Gemini AI agents for SOPs and documentation.",
      tags: ["Jira / Notion Tracking", "Blocker Resolution", "AI Prompt Workflows"]
    },
    {
      step: "04",
      title: "Deployment & Handoff",
      desc: "Managing client review feedback, coordinating final revisions, executing deployment checks, and providing post-launch support.",
      tags: ["Stakeholder Sign-Off", "QA Verification", "Post-Launch Support"]
    }
  ],
  caseStudies: [
    {
      title: "17+ Concurrent Website Projects",
      company: "Think Digital",
      period: "Jan 2026 – Present",
      role: "Project Coordinator",
      desc: "Coordinated multi-client web deployments by managing timeline dependencies, resolving technical blockers, and conducting client feedback loops.",
      highlights: ["Zero missed launch deadlines", "Structured client communication", "Task priority alignment"],
      badgeColor: "bg-indigo-500/10 text-indigo-400 border-indigo-500/20"
    },
    {
      title: "15+ Freelancer Network Ecosystem",
      company: "Freelance Project Coordination",
      period: "Sep 2025 – Present",
      role: "Project Coordinator",
      desc: "Built a network of 15+ vetted freelancers for 12 business owners. Handled quotations, task allocation, and weekly progress sync meetings.",
      highlights: ["Matched client-freelancer skills", "Weekly progress syncs", "Quotation & revision management"],
      badgeColor: "bg-cyan-500/10 text-cyan-400 border-cyan-500/20"
    },
    {
      title: "AI-Assisted Workflow Automation",
      company: "Operational Practice",
      period: "Continuous Integration",
      role: "Operations & AI Specialist",
      desc: "Integrated Claude, Gemini, ChatGPT, and AI agents into operational tasks to speed up documentation, status reporting, and requirement breakdowns.",
      highlights: ["40% faster doc generation", "Standardized SOP creation", "Automated status summaries"],
      badgeColor: "bg-emerald-500/10 text-emerald-400 border-emerald-500/20"
    }
  ],
  skillsCategory: [
    {
      category: "Project & Operations Management",
      icon: "kanban",
      items: ["Project Coordination", "Client & Stakeholder Management", "Requirement Gathering", "Client Onboarding", "Task & Timeline Management", "Process Documentation", "Vendor & Freelancer Management", "Quotation Management"]
    },
    {
      category: "Tools & PM Platforms",
      icon: "layout-grid",
      items: ["Jira", "Trello", "GitHub Projects", "Notion Workspaces", "MS Excel", "Google Sheets", "Google Workspace"]
    },
    {
      category: "AI & Automation Workflows",
      icon: "cpu",
      items: ["Claude AI", "Gemini", "ChatGPT", "AI Agents", "AI-Assisted SOP Workflows", "Digital Process Optimization"]
    }
  ],
  experience: [
    {
      company: "THINK DIGITAL",
      role: "Project Coordinator",
      period: "January 2026 – Present",
      location: "Nashik, MH, India",
      bullets: [
        "Coordinated 17+ website projects from requirements through development, deployment and post-launch support.",
        "Managed client requirements and feedback across multiple concurrent projects by clarifying changes and translating business needs into actionable tasks.",
        "Supported timely delivery by tracking tasks, identifying blockers, and coordinating with team members to resolve issues.",
        "Maintained project continuity through regular follow-ups with clients and team members."
      ]
    },
    {
      company: "FREELANCE PROJECT COORDINATION",
      role: "Project Coordinator",
      period: "September 2025 – Present",
      location: "Nashik, MH, India",
      bullets: [
        "Coordinated projects for 12 business owners by gathering requirements, closing engagements, and preparing quotations.",
        "Built and coordinated a network of 15+ freelancers, matching project requirements with relevant skills.",
        "Monitored client and freelancer relationships through weekly project meetings and tracking progress.",
        "Managed project delivery from initial requirements through completion, following up on pending tasks."
      ]
    },
    {
      company: "PROFCYMA CAREER LLP",
      role: "Business Development Executive",
      period: "October 2025 – December 2025",
      location: "Pune, MH, India",
      bullets: [
        "Communicated with prospective clients to understand their requirements and identify suitable solutions.",
        "Managed follow-ups and coordinated information between clients and internal teams to ensure accuracy."
      ]
    }
  ],
  education: {
    institution: "MET's Institute of Engineering (SPPU)",
    degree: "Bachelor of Engineering in Information Technology",
    period: "2020 – 2024",
    location: "Nashik, MH, India"
  },
  languages: [
    { language: "English", proficiency: "Fully Fluent" },
    { language: "Hindi", proficiency: "Native" },
    { language: "Marathi", proficiency: "Native" }
  ]
};
